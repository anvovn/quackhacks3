...
function InboundsSection({inbounds: init}:{inbounds:Inbound[]}) {
  const [inbounds, setInbounds] = useState(init)
  useEffect(()=>setInbounds(init),[init])

  const statusLabel = (s:Inbound["status"]) =>
    s==="delivered"?"Delivered":s==="in-transit"?"In Transit":"Awaiting Shipment"
  const statusType = (s:Inbound["status"]):React.ComponentProps<typeof StatusPill>["type"] =>
    s==="delivered"?"live":s==="in-transit"?"transit":"pending"

  function advance(ref:string) {
    setInbounds(prev=>prev.map(inb=>{
      if(inb.ref!==ref) return inb
      const next:{[k in Inbound["status"]]:Inbound["status"]} = {
        "awaiting-shipment":"in-transit","in-transit":"delivered","delivered":"delivered"
      }
      return {...inb, status:next[inb.status]}
    }))
  }
        ...
        {inbounds.length===0?(
          <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"48px 0",gap:10}}>
            ...
            <div style={{...S.mono,fontSize:11,color:"var(--muted)",textAlign:"center" as const,maxWidth:320,lineHeight:1.7}}>
              Inbound shipments appear here when ChainAgent approves a reorder.
            </div>
          </div>
        ):(<>
          <RowHead cols="1fr 1fr 70px 110px 110px 120px 120px">
            <Th>Reference</Th><Th>SKU</Th><Th>Units</Th><Th>Supplier</Th><Th>Order Date</Th><Th>ETA</Th><Th>Status</Th>
          </RowHead>
          {inbounds.map((inb,i)=>(
            <div key={inb.ref} style={{display:"grid",gridTemplateColumns:"1fr 1fr 70px 110px 110px 120px 120px",gap:10,padding:"12px 18px",borderBottom:i<inbounds.length-1?"1px solid var(--border)":"none",alignItems:"center",background:inb.status==="in-transit"?"rgba(74,158,255,0.03)":"transparent"}}>
              <div>
                <div style={{fontSize:12,fontWeight:500,color:"var(--text)"}}>{inb.ref}</div>
                <div style={{...S.mono,fontSize:9,color:"var(--muted)"}}>PO: {inb.poRef}</div>
              </div>
              <div>
                <div style={{fontSize:12,color:"var(--text)"}}>{inb.sku}</div>
                <div style={{...S.mono,fontSize:9,color:"var(--muted)"}}>{inb.skuId}</div>
              </div>
              <div style={{...S.mono,fontSize:12}}>{inb.units.toLocaleString()}</div>
              <div style={{...S.mono,fontSize:11,color:"var(--muted2)"}}>{inb.supplier}</div>
              <div style={{...S.mono,fontSize:11,color:"var(--muted)"}}>{inb.orderDate}</div>
              <div style={{...S.mono,fontSize:11,color:inb.status==="delivered"?"var(--accent)":"var(--text)"}}>{inb.eta}</div>
              <div style={{display:"flex",alignItems:"center",gap:6}}>
                <StatusPill label={statusLabel(inb.status)} type={statusType(inb.status)}/>
                {inb.status!=="delivered"&&(
                  <Btn style={{fontSize:9,padding:"3px 7px"}} onClick={()=>advance(inb.ref)}>
                    {inb.status==="awaiting-shipment"?"→ Transit":"→ Delivered"}
                  </Btn>
                )}
              </div>
            </div>
